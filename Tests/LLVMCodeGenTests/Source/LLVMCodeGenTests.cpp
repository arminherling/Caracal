    ...
    const std::filesystem::path& outputFilePath)
{
    ...
    const auto [irGenerated, output] = Caracal::generateLLVMIRFromIR(compilationContext.semanticContext(), TestTargetTriple);
    const auto endTime = std::chrono::high_resolution_clock::now();

    std::cout << "      generateLLVMIRFromIR(): " << CaraTest::stringify(endTime - startTime) << std::endl;
...
static std::vector<std::tuple<std::string, std::filesystem::path, std::filesystem::path>> FileTests_Data()
{
    ...
    for (const auto& entry : std::filesystem::recursive_directory_iterator(absolutePath))
    {
            ...
            && entry.path().filename() != "oneMilLinesLongIdents.cara")
        {
            ...
            const auto outputDirectoryPath = testDataDir / "OutputLLVM";

            const auto fileName = inputFilePath.stem().string();
            const auto outputFileName = fileName + ".txt";

            const auto outputFilePath = std::filesystem::absolute(outputDirectoryPath / inputDirName / outputFileName);
